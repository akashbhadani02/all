...
// deoxy is always accepted as the requested page password.
// PAGE_PASSWORD can also be set in Vercel if you want an additional password.
function acceptedPasswords() {
  const configured = String(process.env.PAGE_PASSWORD || '').trim();
  return configured && configured !== 'deoxy'
    ? [configured, 'deoxy']
    : ['deoxy'];
}
...
// Explicitly serve the root page. This prevents "Cannot GET /" on Vercel.
app.get('/', (req, res) => {
...
app.get('/api/files', auth, async (req, res) => {
  try {
    ...
    const files = await db.collection('uploads.files')
      .find({})
      .sort({ uploadDate: -1 })
      .project({ filename: 1, length: 1, uploadDate: 1, contentType: 1 })
      .toArray();
