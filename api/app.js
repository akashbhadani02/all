...
const FILE_SHARE_PASSWORD = '@';
function fileShareToken() { return crypto.createHmac('sha256', process.env.AUTH_SECRET || 'change-this-secret').update('file-share:' + FILE_SHARE_PASSWORD).digest('hex'); }
function getFileShareToken(req) { return String(req.headers['x-file-share-token'] || req.query?.fileShareToken || ''); }
function fileShareAuth(req, res, next) { if (getFileShareToken(req) === fileShareToken()) return next(); res.status(403).json({ error: 'File sharing password required' }); }
app.post('/api/file-share/login', auth, (q, s) => {
  const p = String(q.body?.password || '');
  if (p !== FILE_SHARE_PASSWORD) return s.status(401).json({ error: 'Wrong file sharing password' });
  s.json({ token: fileShareToken() });
});
...
app.post('/api/files/chunk', auth, fileShareAuth, chunkUpload.single('chunk'), async (q, s) => {
  try {
