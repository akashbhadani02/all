...
async function loadAdminPasswords(){const r=await fetch('/api/admin/passwords',{headers:{Authorization:'Bearer '+sessionStorage.getItem('adminToken')}});const d=await r.json();if(!r.ok)throw Error(d.error||'Could not load passwords');document.getElementById('adminPagePassword').value=d.pagePassword;document.getElementById('adminFilePassword').value=d.fileSharePassword;document.getElementById('adminAdminPassword').value=d.adminPassword;document.getElementById('adminFolders').innerHTML=d.folders.length?d.folders.map(f=>`<div class="admin-folder-row"><div class="admin-folder-name">📁 ${escapeHtml(f.name)}${f.protected?' 🔐':''}</div><label>Password<input class="admin-folder-pass" data-id="${escapeHtml(f.id)}" type="text" value="${escapeHtml(f.password)}" placeholder="No password"></label></div>`).join(''):'<div class="admin-status">No folders created yet.</div>';}
async function saveAdminPasswords(){const status=document.getElementById('adminSaveStatus');const folders=[...document.querySelectorAll('.admin-folder-pass')].map(x=>({id:x.dataset.id,password:x.value}));try{const r=await fetch('/api/admin/passwords',{method:'PUT',headers:{'Content-Type':'application/json',Authorization:'Bearer '+sessionStorage.getItem('adminToken')},body:JSON.stringify({pagePassword:document.getElementById('adminPagePassword').value,fileSharePassword:document.getElementById('adminFilePassword').value,adminPassword:document.getElementById('adminAdminPassword').value,folders})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||'Save failed');status.textContent='✅ All passwords saved. New passwords are active now.';sessionStorage.removeItem('fileShareToken');sessionStorage.removeItem('adminToken');}catch(e){status.textContent='❌ '+e.message;}}
...
if (sessionStorage.getItem("linkPageUnlocked") === "true" && sessionStorage.getItem("fileShareToken")) {
